...
def obtenir_visiteurs_csv(nom_fichier_csv, colonne_dates, parse=True, sep=","):
    resultat = {}
    print(f"[Avertissement] Analyse du fichier : {nom_fichier_csv}")
    if parse:
        df = pd.read_csv(nom_fichier_csv, sep=sep, header=0,
                         parse_dates=[colonne_dates])
...
def main():
    if len(sys.argv) == 1:
        print(
            "usage : [parse] [sep:<separateur>] colonne_dates fichier1 [fichier2 ...]")
        return 1
    if sys.argv[1] == "parse":
    ...
    if noms_fichiers:
        for nom_fichier in noms_fichiers:
            ...
            dico_nb_visiteurs = obtenir_visiteurs_csv(
                nom_fichier, int(colonne_dates), parse=parse, sep=sep)
            ...
            df = pd.DataFrame.from_records(
                donnees, columns=["mois", "nb_visiteurs"])
            # print(df)
            nom_nouveau_csv = "nb_visiteurs_" + nom_donnees + ".csv"
            print("On écrit dans :", nom_nouveau_csv)
            # On separera toujours avec des points-virgules
            df.to_csv(nom_nouveau_csv, sep=";", index=False)
